@extends('layouts.app')
@section('title', 'Agregar Base de Datos')



@section('content')
<div class="container-fluid cm-container-white">
   <h2 style="margin-top:0;display:inline;"><i class="fas fa-plus-circle"></i> Agregar Base de Datos </h2>
 ...
 @endif
   ...
   <form id="newdb" onsubmit="realizaProceso();return false; " >
      @csrf
      ...
      <div class="formularios " id="nuevo_db">
         ...
         <div class="col-sm-6">
            <label>Dominio:</label>
            <input type="text" name="dominio" id="dominio" class="form-control" required="" placeholder="Dominio">
         </div>
...
<script type="text/javascript">
  function realizaProceso() {
         ...
         var formData = new FormData(jQuery('#newdb') [0]); //Se crea el arreglo con los datos del form
         $.ajax({
           url: '{{url('dbs')}}', // Al controlador donde van mis datos
           type: 'POST',
           contentType: false,
         ...
         .done(function(data, textStatus, jqXHR) {
           ...
           if(data.status=='ok'){ //ver controlador, status es el nombre la clave cuando se creo
            $("#resultado").html('<div class="alert alert-success">'+data.mensaje+'</div>');
             document.getElementById("newdb").reset();
           }else{
